			...
			if($validation->passed()) {
				...
				foreach($validation->errors() as $error){
					...
					if(!empty(array_intersect($words, array_map("strtolower", $url_string)))){
						echo "É obrigatório preencher o campo da ideia<br/>";
					}
...
<form action="" method="POST" autocomplete="off" enctype="multipart/form-data">
    ...
    <label for="idea">Pergunta</label>
	  <textarea  maxlength="200" style="
	border:1px solid #ddd; 
	resize: none; " name="idea" class="form-control" id="idea"><?php echo escape(Input::get('idea'));?></textarea>
	  </div>
